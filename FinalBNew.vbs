...
With CMD
.ActiveConnection = DB
.CommandText = "Delete from BillPay_CBS"
End With
CMD.Execute
...
ORS.Open "BillPay_CBS", DB, adOpenStatic, adLockOptimistic 

Do Until OTF.AtEndOfStream
	SNL = OTF.ReadLine
	If Mid(SNL,39,4) = "MBK/" OR Mid(SNL,39,4) = "NEFT" OR Mid(SNL,39,4) = "BILL" OR Mid(SNL,39,4) = "MBK-" Then
				NDate = Mid(SNL,2,10)
...
Set RS = DB.Execute("select Top 1 Balance from BillPay_CBS where NDate = '" & CDate(InDate)-1 &"' order by Serial Desc")
...
Set RS = DB.Execute("select Top 1 Balance from BillPay_CBS where NDate = '" & InDate &"' order by Serial Desc")
